...
func ExampleNewClient() {
	rdb := dicedb.NewClient(&dicedb.Options{
		Addr:     "localhost:7379", // use default Addr
...
func ExampleParseURL() {
	opt, err := dicedb.ParseURL("redis://:qwerty@localhost:7379/1?dial_timeout=5s")
	if err != nil {
		panic(err)
	}
