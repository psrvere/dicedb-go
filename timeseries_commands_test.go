...
var _ = Describe("RedisTimeseries commands", Label("timeseries"), func() {
	...
	BeforeEach(func() {
		client = dicedb.NewClient(&dicedb.Options{Addr: rediStackAddr})
	...
	It("should TSCreate and TSCreateWithArgs", Label("timeseries", "tscreate", "tscreateWithArgs"), func() {
		...
		opt := &dicedb.TSOptions{Retention: 5}
		result, err = client.TSCreateWithArgs(ctx, "2", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		opt = &dicedb.TSOptions{Labels: map[string]string{"Redis": "Labs"}}
		result, err = client.TSCreateWithArgs(ctx, "3", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		opt = &dicedb.TSOptions{Labels: map[string]string{"Time": "Series"}, Retention: 20}
		...
		// Test chunk size
		opt = &dicedb.TSOptions{ChunkSize: 128}
		...
		for _, dup := range duplicate_policies {
			keyName := "ts-dup-" + dup
			opt = &dicedb.TSOptions{DuplicatePolicy: dup}
	...
	It("should TSAdd and TSAddWithArgs", Label("timeseries", "tsadd", "tsaddWithArgs"), func() {
		...
		// Test TSAddWithArgs
		opt := &dicedb.TSOptions{Retention: 10}
		...
		opt = &dicedb.TSOptions{Labels: map[string]string{"Redis": "Labs", "Time": "Series"}, Retention: 10}
		...
		// Test duplicate policy
		// LAST
		opt = &dicedb.TSOptions{DuplicatePolicy: "LAST"}
		...
		// FIRST
		opt = &dicedb.TSOptions{DuplicatePolicy: "FIRST"}
		...
		// MAX
		opt = &dicedb.TSOptions{DuplicatePolicy: "MAX"}
		...
		Expect(resultGet.Value).To(BeEquivalentTo(10))
		// MIN
		opt = &dicedb.TSOptions{DuplicatePolicy: "MIN"}
	...
	It("should TSAlter", Label("timeseries", "tsalter"), func() {
		...
		opt := &dicedb.TSAlterOptions{Retention: 10}
		...
		opt = &dicedb.TSAlterOptions{Labels: map[string]string{"Time": "Series"}}
		resultAlter, err = client.TSAlter(ctx, "1", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultAlter).To(BeEquivalentTo("OK"))
		...
		Expect(resultInfo["labels"].(map[interface{}]interface{})["Time"]).To(BeEquivalentTo("Series"))
		Expect(resultInfo["retentionTime"]).To(BeEquivalentTo(10))
		Expect(resultInfo["duplicatePolicy"]).To(BeEquivalentTo(dicedb.Nil))
		opt = &dicedb.TSAlterOptions{DuplicatePolicy: "min"}
		resultAlter, err = client.TSAlter(ctx, "1", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultAlter).To(BeEquivalentTo("OK"))
	...
	It("should TSCreateRule and TSDeleteRule", Label("timeseries", "tscreaterule", "tsdeleterule"), func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		result, err = client.TSCreateRule(ctx, "1", "2", dicedb.Avg, 100).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		for i := 0; i < 50; i++ {
	...
	It("should TSIncrBy, TSIncrByWithArgs, TSDecrBy and TSDecrByWithArgs", Label("timeseries", "tsincrby", "tsdecrby", "tsincrbyWithArgs", "tsdecrbyWithArgs"), func() {
		...
		opt := &dicedb.TSIncrDecrOptions{Timestamp: 5}
		...
		opt = &dicedb.TSIncrDecrOptions{Timestamp: 7}
		...
		opt = &dicedb.TSIncrDecrOptions{Timestamp: 15}
		...
		opt = &dicedb.TSIncrDecrOptions{ChunkSize: 128}
	...
	It("should TSGet Latest", Label("timeseries", "tsgetlatest", "NonRedisEnterprise"), func() {
		...
		resultGet, err = client.TSCreateRule(ctx, "tsgl-1", "tsgl-2", dicedb.Sum, 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		result, errGet = client.TSGetWithArgs(ctx, "tsgl-2", &dicedb.TSGetOptions{Latest: true}).Result()
		Expect(errGet).NotTo(HaveOccurred())
	...
	It("should TSMGet and TSMGetWithArgs", Label("timeseries", "tsmget", "tsmgetWithArgs", "NonRedisEnterprise"), func() {
		opt := &dicedb.TSOptions{Labels: map[string]string{"Test": "This"}}
		resultCreate, err := client.TSCreateWithArgs(ctx, "a", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		opt = &dicedb.TSOptions{Labels: map[string]string{"Test": "This", "Taste": "That"}}
		...
		mgetOpt := &dicedb.TSMGetOptions{WithLabels: true}
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		opt = &dicedb.TSOptions{Labels: map[string]string{"is_compaction": "true"}}
		resultCreate, err = client.TSCreateWithArgs(ctx, "d", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		resultCreateRule, err := client.TSCreateRule(ctx, "c", "d", dicedb.Sum, 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		mgetOpt = &dicedb.TSMGetOptions{Latest: true}
	...
	It("should TSDel and TSRange", Label("timeseries", "tsdel", "tsrange"), func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange).To(BeEquivalentTo([]dicedb.TSTimestampValue{}))

		resultRange, err = client.TSRange(ctx, "a", 22, 22).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 22, Value: 1}))
	})
	...
	It("should TSRange, TSRangeWithArgs", Label("timeseries", "tsrange", "tsrangeWithArgs", "NonRedisEnterprise"), func() {
		...
		opt := &dicedb.TSRangeOptions{FilterByTS: fts, FilterByValue: []int{1, 2}}
		result, err = client.TSRangeWithArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "+"}
		result, err = client.TSRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 0, Value: 10}, {Timestamp: 10, Value: 1}}))
		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "5"}
		result, err = client.TSRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 0, Value: 5}, {Timestamp: 5, Value: 6}}))
		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Twa, BucketDuration: 10}
		result, err = client.TSRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 0, Value: 2.55}, {Timestamp: 10, Value: 3}}))
		// Test Range Latest
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		resultRule, err := client.TSCreateRule(ctx, "t1", "t2", dicedb.Sum, 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 1, Value: 1}))

		opt = &dicedb.TSRangeOptions{Latest: true}
		resultRange, err = client.TSRangeWithArgs(ctx, "t2", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 0, Value: 4}))
		...
		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRangeWithArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 10, Value: 4}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))

		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10, BucketTimestamp: "+"}
		resultRange, err = client.TSRangeWithArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 20, Value: 4}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))
		// Test Empty
		...
		opt = &dicedb.TSRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10, Empty: true}
		resultRange, err = client.TSRangeWithArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 10, Value: 4}))
	...
	It("should TSRevRange, TSRevRangeWithArgs", Label("timeseries", "tsrevrange", "tsrevrangeWithArgs", "NonRedisEnterprise"), func() {
		...
		opt := &dicedb.TSRevRangeOptions{Aggregator: dicedb.Avg, BucketDuration: 10}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(20))

		opt = &dicedb.TSRevRangeOptions{Count: 10}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &dicedb.TSRevRangeOptions{FilterByTS: fts, FilterByValue: []int{1, 2}}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))

		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "+"}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 10, Value: 1}, {Timestamp: 0, Value: 10}}))

		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "1"}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 1, Value: 10}, {Timestamp: 0, Value: 1}}))

		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Twa, BucketDuration: 10}
		result, err = client.TSRevRangeWithArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]dicedb.TSTimestampValue{{Timestamp: 10, Value: 3}, {Timestamp: 0, Value: 2.55}}))
		// Test Range Latest
		resultCreate, err := client.TSCreate(ctx, "t1").Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 0, Value: 4}))
		opt = &dicedb.TSRevRangeOptions{Latest: true}
		resultRange, err = client.TSRevRangeWithArgs(ctx, "t2", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 10, Value: 8}))
		...
		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRevRangeWithArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 70, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))

		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10, BucketTimestamp: "+"}
		resultRange, err = client.TSRevRangeWithArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 80, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))
		// Test Empty
		_, errAdd = client.TSAdd(ctx, "t4", 15, 1).Result()
		Expect(errAdd).NotTo(HaveOccurred())
		...
		opt = &dicedb.TSRevRangeOptions{Aggregator: dicedb.Max, Align: 0, BucketDuration: 10, Empty: true}
		resultRange, err = client.TSRevRangeWithArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(dicedb.TSTimestampValue{Timestamp: 70, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(7))
	...
	It("should TSMRange and TSMRangeWithArgs", Label("timeseries", "tsmrange", "tsmrangeWithArgs"), func() {
		createOpt := &dicedb.TSOptions{Labels: map[string]string{"Test": "This", "team": "ny"}}
		resultCreate, err := client.TSCreateWithArgs(ctx, "a", createOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		createOpt = &dicedb.TSOptions{Labels: map[string]string{"Test": "This", "Taste": "That", "team": "sf"}}
		...
		// Test Count
		mrangeOpt := &dicedb.TSMRangeOptions{Count: 10}
		...
		mrangeOpt = &dicedb.TSMRangeOptions{Aggregator: dicedb.Avg, BucketDuration: 10}
		...
		mrangeOpt = &dicedb.TSMRangeOptions{WithLabels: true}
		result, err = client.TSMRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][0]).To(BeEquivalentTo(map[interface{}]interface{}{"Test": "This", "team": "ny"}))
		// Test SelectedLabels
		mrangeOpt = &dicedb.TSMRangeOptions{SelectedLabels: []interface{}{"team"}}
		result, err = client.TSMRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		mrangeOpt = &dicedb.TSMRangeOptions{FilterByTS: fts, FilterByValue: []int{1, 2}}
		result, err = client.TSMRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][2]).To(BeEquivalentTo([]interface{}{[]interface{}{int64(15), 1.0}, []interface{}{int64(16), 2.0}}))
		// Test GroupBy
		mrangeOpt = &dicedb.TSMRangeOptions{GroupByLabel: "Test", Reducer: "sum"}
		...
		mrangeOpt = &dicedb.TSMRangeOptions{GroupByLabel: "Test", Reducer: "max"}
		result, err = client.TSMRangeWithArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		mrangeOpt = &dicedb.TSMRangeOptions{GroupByLabel: "team", Reducer: "min"}
		result, err = client.TSMRangeWithArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		...
		// Test Align
		mrangeOpt = &dicedb.TSMRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "-"}
		...
		mrangeOpt = &dicedb.TSMRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: 5}
		result, err = client.TSMRangeWithArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSMRangeWithArgs Latest", Label("timeseries", "tsmrangeWithArgs", "tsmrangelatest", "NonRedisEnterprise"), func() {
		...
		resultCreateRule, err := client.TSCreateRule(ctx, "a", "b", dicedb.Sum, 10).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreateRule).To(BeEquivalentTo("OK"))
		resultCreateRule, err = client.TSCreateRule(ctx, "c", "d", dicedb.Sum, 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = client.TSAdd(ctx, "c", 13, 1).Result()
		Expect(err).NotTo(HaveOccurred())
		mrangeOpt := &dicedb.TSMRangeOptions{Latest: true}
	...
	It("should TSMRevRange and TSMRevRangeWithArgs", Label("timeseries", "tsmrevrange", "tsmrevrangeWithArgs"), func() {
		...
		Expect(len(result["a"][2].([]interface{}))).To(BeEquivalentTo(100))
		// Test Count
		mrangeOpt := &dicedb.TSMRevRangeOptions{Count: 10}
		...
		mrangeOpt = &dicedb.TSMRevRangeOptions{Aggregator: dicedb.Avg, BucketDuration: 10}
		...
		// Test WithLabels
		Expect(result["a"][0]).To(BeEquivalentTo(map[interface{}]interface{}{}))
		mrangeOpt = &dicedb.TSMRevRangeOptions{WithLabels: true}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][0]).To(BeEquivalentTo(map[interface{}]interface{}{"Test": "This", "team": "ny"}))
		// Test SelectedLabels
		mrangeOpt = &dicedb.TSMRevRangeOptions{SelectedLabels: []interface{}{"team"}}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][0]).To(BeEquivalentTo(map[interface{}]interface{}{"team": "ny"}))
		Expect(result["b"][0]).To(BeEquivalentTo(map[interface{}]interface{}{"team": "sf"}))
		...
		mrangeOpt = &dicedb.TSMRevRangeOptions{FilterByTS: fts, FilterByValue: []int{1, 2}}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][2]).To(BeEquivalentTo([]interface{}{[]interface{}{int64(16), 2.0}, []interface{}{int64(15), 1.0}}))
		// Test GroupBy
		mrangeOpt = &dicedb.TSMRevRangeOptions{GroupByLabel: "Test", Reducer: "sum"}
		...
		mrangeOpt = &dicedb.TSMRevRangeOptions{GroupByLabel: "Test", Reducer: "max"}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		mrangeOpt = &dicedb.TSMRevRangeOptions{GroupByLabel: "team", Reducer: "min"}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		...
		// Test Align
		mrangeOpt = &dicedb.TSMRevRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: "-"}
		...
		mrangeOpt = &dicedb.TSMRevRangeOptions{Aggregator: dicedb.Count, BucketDuration: 10, Align: 1}
		result, err = client.TSMRevRangeWithArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSMRevRangeWithArgs Latest", Label("timeseries", "tsmrevrangeWithArgs", "tsmrevrangelatest", "NonRedisEnterprise"), func() {
		...
		_, err = client.TSAdd(ctx, "c", 13, 1).Result()
		Expect(err).NotTo(HaveOccurred())
		mrangeOpt := &dicedb.TSMRevRangeOptions{Latest: true}
