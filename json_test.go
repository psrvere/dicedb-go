		...
		for _, protocol := range protocols {
			When("using protocol version", func() {
				...
				It("should perform complex JSON and RediSearch operations", func() {
					...
					start := 0
					stop := 1
					arrTrimCmd := client.JSONArrTrimWithArgs(ctx, "person:1", "$.person.friends", &dicedb.JSONArrTrimArgs{Start: start, Stop: &stop})
					...
					searchCmd := client.FTSearchWithArgs(ctx, "person_idx", "@contact_value:(alice\\@example\\.com alice_wonder)", &dicedb.FTSearchOptions{Return: []dicedb.FTSearchReturn{{FieldName: "$.person.name"}, {FieldName: "$.person.age"}, {FieldName: "$.person.address.city"}}})
